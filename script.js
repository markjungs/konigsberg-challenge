...
canvas.addEventListener("click", e => {
  ...
  if (currentLand && clickedNode) {
    ...
    if (!gameOver && usedBridges.length === bridges.length) {
      const landDegree = { A:0, B:0, C:0, D:0 };

       bridges.forEach(b => {
         const land1 = b.from.charAt(0);   // A from A1
         const land2 = b.to.charAt(0);     // B from B5
         landDegree[land1]++;
          landDegree[land2]++;
    });

      const oddLands = Object.values(landDegree).filter(d => d % 2 !== 0);

      gameOver = true;
  if (oddLands.length === 0)
   statusText.textContent = "🎉 Eulerian circuit completed!";
  else if (oddLands.length === 2)
  statusText.textContent = "🎉 Eulerian path completed!";
  else
  statusText.textContent = "⚠️ No valid Eulerian path!";
...
resetBtn.addEventListener("click", () => {
  ...
  function checkGraphSolvability() {
  const landDegree = { A: 0, B: 0, C: 0, D: 0 };

  bridges.forEach(b => {
    const land1 = b.from.charAt(0);
    const land2 = b.to.charAt(0);
    landDegree[land1]++;
    landDegree[land2]++;
  });

  const oddCount = Object.values(landDegree).filter(d => d % 2 !== 0).length;

  if (oddCount === 0) {
    return { solvable: true, type: "circuit" }; 
  } else if (oddCount === 2) {
    return { solvable: true, type: "path" };    
  } else {
    return { solvable: false, type: "none" };   
  }
}

const eulerianCheckBtn = document.getElementById("eulerianCheckBtn");

eulerianCheckBtn.addEventListener("click", () => {
  const sol = checkGraphSolvability();

  if (!sol.solvable) {
    statusText.textContent = "⚠️ This graph has NO Eulerian path or circuit.";
  } 
  else if (sol.type === "circuit") {
    statusText.textContent = "✔️ This graph has an Eulerian CIRCUIT.";
  } 
  else if (sol.type === "path") {
    statusText.textContent = "✔️ This graph has an Eulerian PATH.";
  }
});

function getEulerianSolution() {
  
  const adj = { A: [], B: [], C: [], D: [] };

  bridges.forEach(b => {
    const u = b.from.charAt(0);
    const v = b.to.charAt(0);
    adj[u].push({ to: v, id: b.num });
    adj[v].push({ to: u, id: b.num });
  });


  const landDegree = {};
  Object.keys(adj).forEach(k => landDegree[k] = adj[k].length);

  let start = "A"; 
  const odd = Object.keys(landDegree).filter(k => landDegree[k] % 2 !== 0);
  ...
  while (stack.length > 0) {
    ...
    const nextIndex = edges.findIndex(e => !used.has(e.id));
...
showBtn.addEventListener("click", () => {
  ...
  if (!sol.solvable) {
    statusText.textContent = "❌ No solution exists for this graph.";
    return;
  }

  const path = getEulerianSolution();

  if (!path) {
    statusText.textContent = "❌ Graph is unsolvable.";
    return;
  }

  statusText.textContent = "🧭 Solution: " + path.join(" → ");
});
